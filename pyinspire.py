...
def get_text_from_inspire(search="", resultformat="brief", ot=None, max_number_results = 10000):
    ...
    count = 0
    for record_number in range(1, max_number_results, 250):
        data = query_inspire(search, resultformat=resultformat, ot=ot, record_number = record_number)

        if resultformat == "marcxml" or  resultformat == "json":
            if data.decode("utf-8") <= 0: break#print(data.decode("utf-8"))
            count = count + data.decode("utf-8")
        else:
            count = count + extract_from_data(data)
            if extract_from_data(data) <= 0: break #print(extract_from_data(data))
    #print("text = ", text)
    return count
...
def inspire_url(search="", resultformat="brief", startrecord=0, ot=None):
    ...
    inspireoptions = dict(action_search="Search",
                          rg=250, #number of results to return in one page
...
def query_inspire(search="", resultformat="brief", ot=None, record_number = 0):
    ...
    url = inspire_url(search, resultformat, startrecord = record_number ,ot=ot)
...
def extract_text(soup):
    ...
    #print("len = ", len(mainbodies))
    #print("len = ", len(moreinfos))
